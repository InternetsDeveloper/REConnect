...
deploy_cluster() {
    ...
    make_task_def
    register_definition
    if [[ $(aws ecs update-service --cluster worthclark-ui-cf --service worthclark-ui-cf --task-definition $revision | \
    ...
    for attempt in {1..30}; do
        if stale=$(aws ecs describe-services --cluster worthclark-ui-cf --services worthclark-ui-cf | \
